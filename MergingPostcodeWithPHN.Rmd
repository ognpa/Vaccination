---
title: "AIHW2011to2016 - exploring immunisation coverage rates by postcode"
author: "Alex Brooks"
...
clean_immunisation_data <- function(imm_data) {
  ...
  #Age has an unnecessary " year" at the end of each entry. Remove everything after the first space so we can treat age as a number 
  ...
  #There's some values that mean the same but are typed differently. Let's clean those up
...
merge_phn_code <- function(imm_data, phn_data) {
  ...
  for(i in 1:nrow(imm_data)) {
    ...
    imm_data$PHN_code[i] = phn_data$PHN_CODE_2017[1]
    #In case we want a cleaner version, also store just the PHN_number
    imm_data$PHN_number[i] = phn_data$PHN_number[1]
  }
...
phn <- read_csv('raw_data/PostalAreaCodeToPHN.csv')
...
immunisation_data <- merge_phn_code(immunisation_data, phn)
...
##Now, let's learn a little bit more about the data with a few plots

```{r}
#Calculate how many percent of each % immunisanied from <70 to 100
...
## And PHN geographic boundaries change too
