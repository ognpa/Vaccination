...
all_seifa=all_seifa[all_seifa$state=='NSW',]
pcc=read.csv("~/Desktop/pcc.csv")
head(pcc)
all_seifa=merge(all_seifa,pcc,by.x ='postcode',by.y='postcode',all.x=TRUE )
...
train1=all[all$src=='train',-c(47,48)]
test1=all[all$src=='test',-c(47,48)]

train_y=all[all$src=='train','target']
test_y=all[all$src=='test','target']
library(Matrix)

train_sparse <- sparse.model.matrix(~ pcc+IRSAD_URP,train1)
test_sparse <- sparse.model.matrix(~  pcc+IRSAD_URP ,test1)
